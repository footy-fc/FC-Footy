...
function generateAllPayingContributorsScores() {
    console.log("📄 All Paying Contributors & Their SCORES Holdings");
    console.log("==================================================\n");
    ...
    console.log('const allPayingContributorsScores: { address: string; paid: string; scoresHeld: string }[] = [');
    ...
    combinedData.forEach((item, index) => {
        ...
        console.log(`    { address: "${item.address}", paid: "${item.paid}", scoresHeld: "${item.scoresHeld}" }${comma} // ${status} - ${item.percentage}%`);
    });
    ...
    combinedData.slice(0, 10).forEach((item, index) => {
        const status = parseFloat(item.scoresHeld) > 0 ? 'HOLDING' : 'SOLD';
        console.log(`${index + 1}. ${item.address}: ${item.paid} ETH paid, ${parseFloat(item.scoresHeld).toLocaleString()} SCORES held (${status})`);
    });
    
    // Top 10 by SCORES held
    ...
    sortedByScores.slice(0, 10).forEach((item, index) => {
        if (parseFloat(item.scoresHeld) > 0) {
            console.log(`${index + 1}. ${item.address}: ${parseFloat(item.scoresHeld).toLocaleString()} SCORES (${item.percentage}%) - paid ${item.paid} ETH`);
        }
    });
}

generateAllPayingContributorsScores();
