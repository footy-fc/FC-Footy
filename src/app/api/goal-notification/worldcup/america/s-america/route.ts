...
export async function POST(request: NextRequest) {
  const scoreboardUrl =
    "https://site.web.api.espn.com/apis/site/v2/sports/soccer/fifa.worldq.conmebol/scoreboard";
  ...
  try {
    ...
    console.log(`Found ${liveEvents.length} live or completed event(s) in CONMEBOL.`);
  } catch (error) {
    console.error("Error fetching CONMEBOL scoreboard:", error);
    return errorAsOk("Failed to fetch scoreboard", { league: "worldcup-conmebol" });
  }
  ...
  for (const event of liveEvents) {
    ...
    try {
      ...
      console.error(`Error fetching fans for CONMEBOL match ${matchId}`, err);
    }
    ...
    try {
      notificationFlags = await redis.hgetall(`fc-footy:conmebol:notifications:${matchId}`);
    } catch (err) {
    ...
    if (
      ...
      try {
        await redis.hset(`fc-footy:conmebol:notifications:${matchId}`, {
    ...
    try {
      previousScore = await redis.hgetall(`fc-footy:conmebol:match:${matchId}`);
    } catch (err) {
      console.error(`Error fetching Redis data for conmebol match ${matchId}`, err);
      continue;
    }

    if (!previousScore || Object.keys(previousScore).length === 0) {
      console.log(
        `Initializing Redis for conmebol match ${matchId} with scores: ${homeScore}-${awayScore}`
      );
      try {
        await redis.hset(`fc-footy:conmebol:match:${matchId}`, { homeScore, awayScore });
      } catch (err) {
    ...
    console.log(`Goal detected in conmebol match ${matchId}: ${message}`);

    console.log(`Notifying ${fidsToNotify.length} fans for conmebol match ${matchId}`);
