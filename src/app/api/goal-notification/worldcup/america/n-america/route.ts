...
export async function POST(request: NextRequest) {
  const scoreboardUrl =
    "https://site.web.api.espn.com/apis/site/v2/sports/soccer/fifa.worldq.concacaf/scoreboard";
  ...
  try {
    ...
    console.log(`Found ${liveEvents.length} live or completed event(s) in CONCACAF.`);
  } catch (error) {
    console.error("Error fetching CONCACAF scoreboard:", error);
    return errorAsOk("Failed to fetch scoreboard", { league: "worldcup-concacaf" });
  }
  ...
  for (const event of liveEvents) {
    ...
    try {
      ...
      console.error(`Error fetching fans for CONCACAF match ${matchId}`, err);
    }
    const uniqconcacafnsToNotify = new Set([...homeFans, ...awayFans]);
    const fidsToNotify = Array.from(uniqconcacafnsToNotify);
    ...
    try {
      notificationFlags = await redis.hgetall(`fc-footy:concacaf:notifications:${matchId}`);
    } catch (err) {
    ...
    if (
      ...
      try {
        await redis.hset(`fc-footy:concacaf:notifications:${matchId}`, {
    ...
    try {
      previousScore = await redis.hgetall(`fc-footy:concacaf:match:${matchId}`);
    } catch (err) {
      console.error(`Error fetching Redis data for concacaf match ${matchId}`, err);
      continue;
    }

    if (!previousScore || Object.keys(previousScore).length === 0) {
      console.log(
        `Initializing Redis for concacaf match ${matchId} with scores: ${homeScore}-${awayScore}`
      );
      try {
        await redis.hset(`fc-footy:concacaf:match:${matchId}`, { homeScore, awayScore });
      } catch (err) {
    ...
    console.log(`Goal detected in concacaf match ${matchId}: ${message}`);

    console.log(`Notifying ${fidsToNotify.length} fans for concacaf match ${matchId}`);
