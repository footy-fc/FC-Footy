...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`Found ${liveEvents.length} live or completed event(s) in FIFA World Cup.`);
  } catch (error) {
    console.error("Error fetching FIFA World Cup scoreboard:", error);
    return errorAsOk("Failed to fetch scoreboard", { league: "worldcup" });
  }
  ...
  for (const event of liveEvents) {
    ...
    console.log(`Processing World Cup match ID: ${matchId}`);
    ...
    try {
      ...
      console.error(`Error fetching fans for World Cup match ${matchId}`, err);
    }
    ...
    try {
      notificationFlags = await redis.hgetall(`fc-footy:worldcup:notifications:${matchId}`);
    } catch (err) {
    ...
    if (
      ...
      await sendFrameNotificationsBatch({
        fids: fidsToNotify,
        title: "Match Started! (WORLD CUP)",
        body: message,
      });

      try {
        await redis.hset(`fc-footy:worldcup:notifications:${matchId}`, {
    ...
    try {
      previousScore = await redis.hgetall(`fc-footy:worldcup:match:${matchId}`);
    } catch (err) {
      console.error(`Error fetching Redis data for World Cup match ${matchId}`, err);
      continue;
    }

    if (!previousScore || Object.keys(previousScore).length === 0) {
      console.log(
        `Initializing Redis for World Cup match ${matchId} with scores: ${homeScore}-${awayScore}`
      );
      try {
        await redis.hset(`fc-footy:worldcup:match:${matchId}`, { homeScore, awayScore });
      } catch (err) {
    ...
    const message = `${
      ...
      awayTeam.team?.shortDisplayName || awayTeam.team?.displayName
    } | ${scoringPlayer} scored at ${clockTime} (WORLD CUP)`;
    goalNotifications.push(message);
    console.log(`Goal detected in World Cup match ${matchId}: ${message}`);

    await sendFrameNotificationsBatch({
      fids: fidsToNotify,
      title: "Goal! Goal! Goal! (WORLD CUP)",
      body: message,
    });
