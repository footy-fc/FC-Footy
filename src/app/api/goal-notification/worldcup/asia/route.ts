...
export async function POST(request: NextRequest) {
  const scoreboardUrl =
    "https://site.web.api.espn.com/apis/site/v2/sports/soccer/fifa.worldq.afc/scoreboard";
  ...
  try {
    ...
    console.log(`Found ${liveEvents.length} live or completed event(s) in AFC.`);
  } catch (error) {
    console.error("Error fetching AFC scoreboard:", error);
    return errorAsOk("Failed to fetch scoreboard", { league: "worldcup-afc" });
  }
  ...
  for (const event of liveEvents) {
    ...
    try {
      ...
      console.error(`Error fetching fans for AFC match ${matchId}`, err);
    }
    ...
    try {
      notificationFlags = await redis.hgetall(`fc-footy:afc:notifications:${matchId}`);
    } catch (err) {
    ...
    if (
      ...
      try {
        await redis.hset(`fc-footy:afc:notifications:${matchId}`, {
    ...
    try {
      previousScore = await redis.hgetall(`fc-footy:afc:match:${matchId}`);
    } catch (err) {
      console.error(`Error fetching Redis data for afc match ${matchId}`, err);
      continue;
    }

    if (!previousScore || Object.keys(previousScore).length === 0) {
      console.log(
        `Initializing Redis for afc match ${matchId} with scores: ${homeScore}-${awayScore}`
      );
      try {
        await redis.hset(`fc-footy:afc:match:${matchId}`, { homeScore, awayScore });
      } catch (err) {
    ...
    console.log(`Goal detected in afc match ${matchId}: ${message}`);

    console.log(`Notifying ${fidsToNotify.length} fans for afc match ${matchId}`);
