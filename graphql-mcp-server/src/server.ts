...
class GraphQLMCPServer {
      ...
      try {
        switch (name) {
          ...
          case "get_bendystraw_projects":
            return await this.getBendystrawProjects(args as any);
          case "get_bendystraw_project":
            if (!args?.projectId || !args?.chainId) {
              throw new Error("Project ID and Chain ID are required");
            }
            return await this.getBendystrawProject(args.projectId as number, args.chainId as number);
          case "get_bendystraw_sucker_groups":
            return await this.getBendystrawSuckerGroups(args as any);
          case "get_bendystraw_participants":
            ...
            return await this.getBendystrawParticipants(args.suckerGroupId as string, args.timestamp as number);
          default:
  ...
  private async getBendystrawProjects(args: any) {
  ...
  private async getBendystrawProject(projectId: number, chainId: number) {
    const query = `
  ...
  private async getBendystrawSuckerGroups(args: any) {
    const limit = args.limit || 10;
    const orderBy = args.orderBy || "createdAt";
    const orderDirection = args.orderDirection || "desc";
  ...
  private async getBendystrawParticipants(suckerGroupId: string, timestamp: number) {
